...
DEFAULT_BUTTON_POSITIONS = {
    'print': (20, 50),   # Adjust based on your toolbar layout
    'save': (60, 50),    # Adjust based on your toolbar layout
    'zoom_in': (100, 50),
    'zoom_out': (140, 50),
}
...
def detect_button_regions_with_opencv(screenshot, log_file):
    """
    Use OpenCV to detect button-like regions (rectangular areas that might be buttons).
    This looks for uniform regions that could be toolbar buttons.
    ...
    try:
        ...
        # Focus on top portion of window (toolbar area)
        # Assuming toolbar is in top 100 pixels
        toolbar_height = min(100, img_gray.shape[0])
        toolbar_region = img_gray[0:toolbar_height, :]
        ...
        log(log_file, f"Detected {len(button_regions)} potential button regions")
...
def get_button_coordinates(window, button_name, log_file, detected_regions=None):
    """
    Get coordinates for a button, either from detected regions or default positions.
    
    Args:
        window: Window object from pywinauto
        button_name: Name of button ('save', 'print', etc.)
        log_file: Log file handle
        detected_regions: Optional list of detected button regions from OpenCV
    
    Returns:
        dict with 'center_x', 'center_y' or None
    """
    try:
        window_rect = window.rectangle()
        
        # If we have detected regions and button_name matches a position, use that
        if detected_regions and button_name in ['save', 'print']:
            # Use first or second detected region based on button
            # Assuming save is typically first or second button
            if len(detected_regions) >= 2:
                button_idx = 1 if button_name == 'save' else 0
                region = detected_regions[button_idx]
                log(log_file, f"Using detected region {button_idx} for '{button_name}': "
                    f"({region['center_x']}, {region['center_y']})")
                return region
        
        # Fallback to default positions
        if button_name in DEFAULT_BUTTON_POSITIONS:
            offset_x, offset_y = DEFAULT_BUTTON_POSITIONS[button_name]
            log(log_file, f"Using default position for '{button_name}': ({offset_x}, {offset_y})")
            log(log_file, "NOTE: You may need to adjust DEFAULT_BUTTON_POSITIONS if button is not found")
            return {
                'center_x': offset_x,
                'center_y': offset_y,
                'left': offset_x - 10,
        ...
        log(log_file, f"No coordinates found for button '{button_name}'", "WARNING")
        return None
...
def click_at_coordinates(window, button_info, log_file, monitor=None):
    """
    Click at the specified coordinates relative to the window.
    
    Args:
        window: Window object from pywinauto
        button_info: Dict with 'center_x', 'center_y' relative to window
        log_file: Log file handle
    ...
    try:
        log(log_file, "\n" + "="*60)
        log(log_file, "STEP 3: Clicking button at coordinates")
        log(log_file, "="*60)
        ...
        # button_info coordinates are relative to the window (starting at 0,0 of window)
        ...
        log(log_file, f"✓ Clicked at ({absolute_x}, {absolute_y})")
...
def handle_file_dialog(log_file, save_path):
    """
    Handle the Windows file dialog using pyautogui.
    
    Args:
        log_file: Log file handle
        save_path: Path object where file should be saved
    ...
    try:
        log(log_file, "\n" + "="*60)
        log(log_file, "STEP 4: Handling Windows file dialog")
        log(log_file, "="*60)
        ...
        # Common file dialog has address bar that can be activated with Alt+D or clicking
        # First, try to activate address bar with Alt+D
        pyautogui.hotkey('alt', 'd')
        time.sleep(0.5)
        
        # Type the directory path
        directory_path = str(save_path.parent)
        ...
        # Navigate to filename field (usually activated automatically or with Tab)
        # Try Tab to move to filename field
        pyautogui.press('tab')
        time.sleep(0.3)
        ...
        log(log_file, f"Error handling file dialog: {type(e).__name__}: {str(e)}", "ERROR")
        import traceback
        log(log_file, traceback.format_exc(), "ERROR")
        return False
...
def main():
    ...
    log(log_file, "")
    log(log_file, "NOTE: Button detection uses coordinate-based approach.")
    log(log_file, "If buttons are not found, you may need to adjust DEFAULT_BUTTON_POSITIONS")
    ...
    try:
        ...
        if CV2_AVAILABLE:
            log(log_file, "\n" + "="*60)
            log(log_file, "STEP 2b: Attempting button region detection")
            log(log_file, "="*60)
            detected_regions = detect_button_regions_with_opencv(screenshot, log_file)
        
        # Step 3: Get button coordinates (for Save button)
        # Try 'save' first, fallback to 'print' if save coordinates not found
        button_name = 'save'
        button_info = get_button_coordinates(preview_window, button_name, log_file, detected_regions)
        
        if not button_info:
            log(log_file, f"Could not find coordinates for '{button_name}', trying 'print'...", "WARNING")
            button_name = 'print'
            button_info = get_button_coordinates(preview_window, button_name, log_file, detected_regions)
        ...
        if not click_success:
            log(log_file, "Failed to click button", "ERROR")
            return
        
        # Step 5: Handle file dialog
        # Define save path in testing folder
        save_path = LOG_DIR / "test_item_label.pdf"
        log(log_file, f"Target save path: {save_path}")
        
        dialog_success = handle_file_dialog(log_file, save_path)
        ...
        log(log_file, "If the button was not clicked correctly:")
        log(log_file, "1. Check the saved screenshot in the testing folder")
        log(log_file, "2. Note the pixel coordinates of the Save/Print button")
        log(log_file, "3. Update DEFAULT_BUTTON_POSITIONS in the script:")
        log(log_file, "   DEFAULT_BUTTON_POSITIONS = {")
        log(log_file, "       'save': (X_OFFSET, Y_OFFSET),  # Update with actual coordinates")
        log(log_file, "       'print': (X_OFFSET, Y_OFFSET),")
        log(log_file, "   }")
